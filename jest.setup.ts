...
if (typeof globalThis.Request === 'undefined') { globalThis.Request = class MockRequest {}; }
if (typeof globalThis.Response === 'undefined') { globalThis.Response = class MockResponse {}; }
if (typeof globalThis.Headers === 'undefined') {
   globalThis.Headers = class MockHeaders extends Map<string, string> {
      append(name: string, value: string): void { super.set(name, value); }
      has(name: string): boolean { return super.has(name); }
      get(name: string): string | undefined { return super.get(name); }
      set(name: string, value: string): this { super.set(name, value); return this; }
      delete(name: string): boolean { return super.delete(name); }
      forEach(callback: (value: string, key: string, map: Map<string, string>) => void, thisArg?: any): void { super.forEach(callback, thisArg); }
   };
}
if (typeof globalThis.fetch === 'undefined') { globalThis.fetch = jest.fn(); }
...
    }),
  };
  const MockRatelimitConstructor = jest.fn(() => mockRatelimitInstanceMethods) as jest.Mock & { slidingWindow: jest.Mock; };
  Object.assign(MockRatelimitConstructor, { slidingWindow: jest.fn((limit, duration) => ({ /* return value */ })) });
...
jest.mock('next-auth', () => {
    ...
    const MockNextAuthFunction = jest.fn((options: any) => { // Use 'any' for options type
